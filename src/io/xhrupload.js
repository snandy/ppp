...
var exports = {
	init: function(input, settings) {
		...
		// 允许一次上传的文件数量
		this.maximum  = settings.maximum || 10
		// 允许上传的文件类型 正则
		this.refile = settings.refile || /\S/
		// 跨域带证书
		...
		input.onchange = function(e) {
			...
			function sched() {
				...
				if (!file) {
					...
					i++;
					file.id = me.guid++
					if (file.size > me.maximize) {
						return me.checkMaximize(file)
					}
					if (!me.refile.test(file.type)) {
						return me.checkFileType(file)
					}
	...
	cancel: function() {
		try {
			forEach(this.xhrs, function(xhr) {
				xhr.abort()
			}) 
		} catch(e) {}
