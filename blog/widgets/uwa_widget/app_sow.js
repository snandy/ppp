...
Element.Methods = {
  ...
  removeClassName: function(element, className) {
	...
	getChildElementByTagName: function(element, tagName) {
		...
		var arr = [];	
		for (var i=0; i<ln; i++) {
			if (element.childNodes[i].nodeName == tagName) {arr.push(element.childNodes[i]);}
		}
		return arr;
	},
...
var App = {
	...
	Actions: {
		imgPath: 'http://img3.pp.sohu.com/ppp/blog/styles_ppp/images/'
	},
	
	Widgets: {
	},
	
	maxWidgetLoadTime: 10000
}
...
App.Module.prototype = {
	...
	initialize: function(options, isInit) {
		this.options = Object.extend({
				...
				floating: 	false,			//	未知
				title: 		'',				//	默认标题
				data: 		null,			//	widget初始化参数
				page:		null,			//	未知
				placeIn:	document.body,	//	未知
				placeBefore:null,			//	未知
				newMode:	false			//	未知
			}, options || {});
			
		Object.extend(this.options, {
				editMode:	false,					//	是否能够编辑
				loaded:		false,					//	是否能够刷新
				loadTime:	0,						//	未知
				maxLoadTime:App.maxWidgetLoadTime	//	最长加载时间
			});
			
		this.build();
			
		this.m_content = this.getModuleContent();	//	取得内容区对象
	...
	build: function() {
		...
		if (App.Permit.editModule) {
			...
			if (!this.options.newMode) {
				...
				Event.observe(divClose, 'click', this.eventCancelBubble);
		...
		this.options.placeIn.appendChild(divModule);
	},
	...
	*/
	setEditBtn: function() {
		this.elm_edit.firstChild.innerHTML = "设置";
	},
