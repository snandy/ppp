...
var o_scrap1 = function(m_data, m_content, m_edit){
	var eles = null; 		// Widget 内容展示部分需要填充的对象列表
	var elmOutput = null;	// 内容输出容器
	var blogHost = 'http://'+window.location.host;
	var scrap = blogHost+'/inc/olympic/frag/51/86433_258245195.inc?ts='+(new Date()).getTime();
	...
	this.build = function(nocache) {
		...
		m_content.innerHTML = '';
		elmOutput.innerHTML = '<div></div><div style="text-align:right;display:none;"><a target="_blank" href="http://zt.blog.sohu.com/s2008/5195/s258343402_news/">更多>></a></div>';
		m_content.appendChild(elmOutput);
		this.initEles();
		this.setTitle('博奥论坛', true);
		this.getScrap();
	};
	// 获取所有需要回填数据的对象
	...
	this.getScrap = function(){
		...
		new Ajax.Request(
			url,
			{
				...
				onFailure: this.noData.bind(this,eles.news,'暂无博奥论坛的报道')
			}
		);
	};
