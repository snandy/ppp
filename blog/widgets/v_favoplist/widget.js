...
var v_favoplist = function(m_data, m_content, m_edit, w_path){
		...
		if(wdtData && wdtData.length) {
			for(var i=0; i<wdtData.length; i++) {
				if (wdtData[i]) {
					...
					if (i == 0) {
						str += '\
							<div class="video_cover">\
								<a href="' + dataNow.url + '" title="' + title + '" alt="' + title + '" target="_blank">\
					...
					if (i > 0) {
						str += '<li><a href="' + dataNow.url + '" title="' + title + '" alt="' + title + '" target="_blank">' + title + '</a>（' + App.Lang.v_from + '：<a href="' + dataNow.authorUrl + '" target="_blank" title="' + dataNow.authorName + '" alt="' + dataNow.authorName + '">' + dataNow.authorName + '</a>）</li>';
					}
					if (i == wdtData.length-1 && i != 0) {
						str += '\
							</ul>';
					}
					if (i == wdtData.length-1) {
						str += '\
	...
	this.loadedData = function(canRefresh) {
		...
		if (data.status == 1) {
			...
			outputEle.innerHTML = App.Lang.v_favPlist;
		} else {
