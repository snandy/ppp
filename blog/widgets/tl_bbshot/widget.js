...
var tl_bbshot = function(m_data, m_content, m_edit){
	
	var url = '/rss/tl/hotbbs.xml';
	var keys = ['title', 'name', 'style'];
	...
	this.gotData = function(trans) {
		...
		if (items && items.length) {
			...
			for (var i=0; i<items.length; i++) {
				...
				arr.push('<li>');
				arr.push('<a href="'+ itemData.value +'" target="_blank" title="' + itemData.title + '">' + (itemData.style ? '<strong>' : '') + itemData.title + ' ' + itemData.name + (itemData.style ? '</strong>' : '') + '</a></li>');
			}
			arr.push('</ul>');
		} else {
			arr.push('暂无任何热帖');
