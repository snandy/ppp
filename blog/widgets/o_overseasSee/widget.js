/******* bj2008 Olympic Game overseas news Widget **********/
...
/*******************************************************/
//花絮幕后
var o_overseasSee = function(m_data, m_content, m_edit){
	var eles = null; 		// Widget 内容展示部分需要填充的对象列表
	var elmOutput = null;	// 内容输出容器
	var blogHost = 'http://'+window.location.host;
	var scrap = blogHost+'/inc/olympic/frag/95/86268_commonlist11_258245195.inc?ts='+(new Date()).getTime();
	...
	this.build = function(nocache) {
		...
		m_content.innerHTML = '';
		elmOutput.innerHTML = '<div></div><div style="text-align:right;display:none;"><a target="_blank" href="http://zt.blog.sohu.com/s2008/5195/s258343731/index.shtml">更多>></a></div>';
		m_content.appendChild(elmOutput);
		this.initEles();
		this.setTitle('花絮幕后', true);
		this.getScrap();
	};
	// 获取所有需要回填数据的对象
	...
	this.getScrap = function(){
		var url = scrap;//'这里是花絮幕后的碎片地址';
		new Ajax.Request(
			url,
			{
				method:'get',
				onSuccess: this.showScrap.bind(this),
				onFailure: this.noData.bind(this,eles.news,'暂无花絮幕后')
			}
		);
	};
