...
/*******************************************************/
//独家访谈
var o_scrap3 = function(m_data, m_content, m_edit){
	var eles = null; 		// Widget 内容展示部分需要填充的对象列表
	var elmOutput = null;	// 内容输出容器
	var blogHost = 'http://'+window.location.host;
	var scrap = blogHost+'/inc/olympic/frag/51/86442_258245195.inc?ts='+(new Date()).getTime();
	...
	this.getScrap = function(){
		...
		new Ajax.Request(
			url,
			{
				...
				onFailure: this.noData.bind(this,eles.news,'暂无独家访谈的报道')
			}
		);
	};
