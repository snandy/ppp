...
var v_favovideo = function(m_data, m_content, m_edit, w_path){
		...
		if(wdtData && wdtData.length) {
			for(var i=0; i<wdtData.length; i++) {
				if (wdtData[i]) {
					...
					if (i == 0) {
						str += '\
							<div class="first_fav"><a href="' + dataNow.url + '" title="' + title + '" alt="' + title + '" target="_blank" class="video_cover"><img src="' + (dataNow.cover || defCover) + '" /><span class="v_play"></span></a></div>\
							<dl>\
								...
								<dd>' + App.Lang.v_from + '：<a href="' + dataNow.authorUrl + '" target="_blank" title="' + dataNow.authorName + '" alt="' + dataNow.authorName + '">' + dataNow.authorName + '</a></dd>\
							</dl>\
					...
					if (i > 0) {
						str += '<li><a href="' + dataNow.url + '" title="' + title + '" alt="' + title + '" target="_blank">' + title + '</a></li>';
					}
					if (i == wdtData.length-1 && i != 0) {
						str += '</ul>';
					}
				}
			}
			
			str += '<div class="more"><a href="' + listUrl + '" target="_blank">' + App.Lang.v_more + '</a></div>';
		} else {
			str = App.Lang.v_favVideo;
		}
	...
	this.loadedData = function(canRefresh) {
		...
		if (data.status == 1) {
			...
			outputEle.innerHTML = App.Lang.v_favVideo;
		} else {
