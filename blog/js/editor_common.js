		...
		if(imgElement) {
			...
			if (type == "imgs") {
				...
				for(var i=0;i<imgCount;i++) {
					var dataNow = data[i];
					var url = dataNow.url;
					var alt = dataNow.alt ? dataNow.alt : "";
					var link = dataNow.link;
					if(link) html += '<a href="'+link+'" target="_blank">';
					html += '<img src="'+url+'" border="0" style="'+align+'" alt="'+alt+'" />';
					if(link) html += '</a>';
					if(showMemo) html += '<div style="'+align+'">'+(data[i].alt ? data[i].alt : "")+'</div>';
				}
			} else {
				html = '<br /><!-- sohublogblock begin -->';
				if (info.kind == 1) {
					//	如果为要插入专辑的小图模式
					html += '<div style="float:left;padding:7px;"><div style="float:left;margin:0 10px 8px 0;width:229px;"><div style="padding:10px;background-color:white;text-align:center"><a href="'+info.link+'" target=_blank><img style="width:200px;" src="'+info.url+'"></a><div style="font-size:12px;color:#333;text-align:left;"><div style="font-weight:bold;padding-top:10px;">'+info.name+'</div><div style="padding-top:10px;">'+info.desc+'</div><div style="text-align:right;padding-top:10px;">'+info.count+'张</div></div></div></div>';
					var dataLength = data.length;
					for (var i=0;i<dataLength;i++) {
						var dataNow = data[i];
						html += '<div style="float:left;margin:2px;text-align:center"><a href="'+dataNow.link+'" target=_blank><img alt="'+dataNow.alt+'" src="'+dataNow.url+'" border=0></a></div>';
					}
					
					html += '</div>';
				} else if(info.kind == 2) {
					//	如果为要插入专辑的大图模式
					html += '<div style="overflow:auto;float:left;background-color:#333;padding:7px;"><div style="float:left;margin:0 10px 8px 0;width:300px;"><div style="padding:10px;background-color:white;text-align:center"><a href="'+info.link+'" target=_blank><img style="width:250px;" src="'+info.url+'"></a><div style="font-size:12px;color: #333;text-align:left";><div style="font-weight:bold;padding-top:10px;">'+info.name+'</div><div style="padding-top:10px;">'+info.desc+'</div><div style="padding-top:10px;text-align:right">'+info.count+'张</div></div></div></div>';
					var dataLength = data.length;
					for (var i=0;i<dataLength;i++) {
						var dataNow = data[i];
						html += '<div style="float:left;margin:0 5px 1px 0;width:150px;height:190px;text-align:center;"><div style="margin:0;width:100%;background-color:white;"><div style="padding:7px;text-align:center;"><a href="'+dataNow.link+'" target=_blank><img src="'+dataNow.url+'" alt="'+dataNow.alt+'"></a><div style="font-size:12px;margin:7px 0 0;overflow:hidden;color:#333;height:28px;text-align:left;line-height:120%;">'+dataNow.alt+'</div></div></div></div>';
					}
					
					html += '</div>';
				}
				html += '<div style="clear:both;"></div><!-- sohublogblock end --><br/>';
			}
			Editor.pasteHTML(html);
		}
	}
