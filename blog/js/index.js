...
function drawAppInfo(o) {
        ...
        if (!o) {return;}
        if (isLogin() || (isPPLogin() && getProduct('blog'))) {
			o.innerHTML = '<div style="padding:30px 0 0 80px;"><img src="http://blog.sohu.com/home/new/style/images/loading.gif" alt="" align="absmiddle" /> 正在读取...</div>';
			getAppInfo(o);
        }
        else {
			showAppInfo(o);
			//o.innerHTML = 'cookie验证错误，请重新刷新页面或联系管理员';
        }
}
function getAppInfo(o) {
	flag_gegUserInfo++;
	var dataURL = getD()? 'http://'+ getD() +'.blog.sohu.com/service/profile.jsp' : 'http://blog.sohu.com/service/profile.jsp';
...
function showAppInfo(o) {
	...
	s += '<div id="login_ico"><a href="http://'+getD()+'.blog.sohu.com"><img id="login_icoImg" src="'+ ico +'" alt="'+ (getP() || '头像') +'" /></a></div>';
	...
	if (Browser.ua.indexOf('ie')<0) {
		s += getPPP() || getP();
		//s += '<span id="tb_sohuMsg" class="tb_sohuMsg" title="小纸条">&nbsp;</span>';
	}else{
		s += getShortName(getPPP() || getP());
		s += '<span id="tb_sohuMsgFlash" title="小纸条"><embed src="http://images.chinaren.com/product/webim/mood/mood_blog.swf?UserID='+ this.getP() +'" wmode="transparent" quality="high" pluginspage="htt p://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" menu="-1" allowscriptaccess="always" /></span>';
	}
	s += '</p>';
	s += '<div id="login_opr">';
	if (hasBlog() && getUserType() != 3) {
		s += '<a href="http://blog.sohu.com/manage/main.do?tracker=index" title="进入我的空间" target="_blank">进入我的空间</a>|<a href="http://'+getD()+'.blog.sohu.com" title="查看我的博客" target="_blank">查看我的博客</a>';
	} else {
		s += '<a href="/login/activation.do" title="您还没有激活博客，请先激活" target="_blank">立即激活博客</a>';
	}
	//s += '<a href="http://passport.sohu.com/logout.jsp?appid=1019&ru=http://blog.sohu.com/logout.do">离开</a>';
	o.innerHTML = s;
	o.style.display = '';
...
function showFocusImg() {
	if (typeof focusImgs == 'undefined' || !focusImgs || focusImgs.length==0) {return;}
...
function initProfileSearch() {
	...
	if (typeof(str) == "string" && str.length > 0 && AreaData.loc[str]) {
		...
		if (arr.length > 1) cityId.value = arr[1];
	}
