	...
	doSend: function(mode, obj) {
		...
		switch(mode) {
			...
			case 'pageOrder':
				...
				break;
			/*case 'colStyle':
				url = App.Actions.colStyle;
				options = {
					parameters: 'm=update&cols='+obj,
					data: {okFun: App.ModContainer.doSetColStyle.bind(App.ModContainer)}
				};
				break;*/
			case 'theme':
...
App.Framework = {
		...
		$A(App.WidgetLib.gallery).each(function(g, i) {
			...
			if (g.type == "function") {
				...
				$A(App.WidgetLib.lib).each(function(w) {
					...
					/*if (w.only && !w.ableDel && App.Modules.getObjByType(w.type)) {
						throw $continue;
					}*/
					...
					var wInfo = {
						...
						action: App.Modules.newMod.bind(App.Modules),//this.newModPre.bind(this),
						value: w.type,
						disabled: disabled
					};
					arr2.push(wInfo);
	...
	//	设置分享按钮是否显示
	resetShareTheme: function() {
		if (this.setThemeMenu && this.setThemeMenu.shareThemeBtn) {
			if (App.UserStyle.canShareTheme()) {
				Element.show(this.setThemeMenu.shareThemeBtn);
			} else {
				Element.hide(this.setThemeMenu.shareThemeBtn);
			}
		}
	}*/
};
...
App.PageTab.prototype = {
	...
	regModObservers: function() {
		...
		var opt4sortable = {
			...
			dropOnEmpty: true,
			targeting: true,
			//containment: App.ModContainer.cols.concat(document.body),
...
App.Module.prototype = {
	...
	build: function(event) {
		...
		if (App.Permit.editModule) {
			if (!this.options.newMode) {
				...
				//Element.addClassName(divClose, 'modCls');
				...
				//divClose.innerHTML = '<img src="'+App.Actions.imgPath+'spacer.gif" alt="'+ clsAlt +'" title="'+ clsAlt +'" />';
			...
			//Element.addClassName(divEdit, 'modEdit');
		...
		if (!this.options.newMode) {
			...
			//Element.addClassName(divRefresh, 'modRfs');
			divRefresh.title = App.Lang.refreshModuleBtnTitle;
			//divRefresh.innerHTML = '<img src="'+App.Actions.imgPath+'spacer.gif" alt="'+ 
				//App.Lang.refreshModuleBtnTitle +'" title="'+ App.Lang.refreshModuleBtnTitle +'" />';
	...
	showWidgetInfo: function() {
		/*if (App.WidgetLib.getWidget(this.options.type)) {
			...
			var pubDate = widget.pubDate;
			var js = widget.js;
			var css = widget.css;
		}
		
		var str = '';
		str += '<div style="color: #999;font-size: 10px;"><strong>Widget Information:</strong><hr />';
		str += '<div>';
		str += '<strong>title:</strong> ' + (title || '') +'<br />';
		str += '<strong>description:</strong> ' + (desc || '') +'<br />';
		var icoImg = ico? '<img src="'+ ico +'" />' : '';
		str += '<strong>icon:</strong> ' + icoImg +'('+ (ico || '') +')<br />';
		str += '<strong>author:</strong> ' + (author || '') +'<br />';
		str += '<strong>site:</strong> ' + (site || '') +'<br />';
		str += '<strong>pubDate:</strong> ' + (pubDate || '') +'<br />';
		str += '<strong>js:</strong> ' + (js || '') +'<br />';
		str += '<strong>css:</strong> ' + (css || '') +'<br />';
		str += '</div></div>';*/
		var str = '';
...
App.Widget.prototype = {
			/*title:	'Hello World',
			desc:	'A sample hello world test.',
			ico:	'ico_widget.gif',
			author:	'Todd Lee',
			site:	'http://www.todd-lee.com',
			...
			type:	'Demo_Hello',
			path:	App.Actions.widgetLibPath + 'demo_hello/',
			js:		['hello.js'],
	...
	loadLib: function() {
		...
		$A(this.js).each(function(c){
			(Browser.ua == 'opera') ?
				setTimeout(function(){ var aa = new LinkFile(c, {type: 'script'}); }, 100)
				: new LinkFile(c, {type: 'script'});
		});
	}
...
App.UserStyle = {
	...
	getCusHeaderCon: function() {
		...
		//arr.push('<ul style="list-style:none;margin:5px 0;">');
		//arr.push('<li><input name="ct_header_bgi_type" id="ipt_useThemeDefaultHeaderImg" type="radio" value="1" /><label for="ipt_useThemeDefaultHeaderImg">'+ App.Lang.useThemeDefaultHeaderImg +'</label></li>');
		//arr.push('<li><input name="ct_header_bgi_type" id="ipt_hideHeaderImg" type="radio" value="2" /><label for="ipt_hideHeaderImg">'+ App.Lang.hideHeaderImg +'</label></li>');
		//arr.push('<li><input name="ct_header_bgi_type" id="ipt_useMyHeaderImg" type="radio" value="3" /><label for="ipt_useMyHeaderImg">'+ App.Lang.useMyHeaderImg +'</label> ');
		...
		$A(_ipts).each(function(ipt) {
			/*if (ipt.id == 'ipt_useThemeDefaultHeaderImg') {
				this.ipt_useThemeDefaultHeaderImg = ipt;
			}
			if (ipt.id == 'ipt_hideHeaderImg') {
				this.ipt_hideHeaderImg = ipt;
			}
			if (ipt.id == 'ipt_useMyHeaderImg') {
				this.ipt_useMyHeaderImg = ipt;
			}*/
	...
	observeCusHeader: function() {
		/*Event.observe(this.ipt_useThemeDefaultHeaderImg, 'click', function() {
			this.setCss('#header', 'background-image', '');
		}.bind(this));
		Event.observe(this.ipt_hideHeaderImg, 'click', function() {
			this.setCss('#header', 'background-image', 'none');
		}.bind(this));
		Event.observe(this.ipt_useMyHeaderImg, 'click', function() {
			if (this.getCss('#header-bak', 'background-image')) {
				this.setCss('#header', 'background-image', this.getCss('#header-bak', 'background-image'));
			}
		}.bind(this));*/
		$A(this.elm_header_bgis).each(function(e) {
	...
	getCusBodyBgCon: function() {
		...
		arr.push('<legend>'+ App.Lang.selectImage +'</legend>');
		//arr.push('<ul style="list-style:none;margin:5px 0;">');
		//arr.push('<li><input name="ct_body_bgi_type" id="ipt_useThemeDefaultBodyImg" type="radio" value="1" /><label for="ipt_useThemeDefaultBodyImg">'+ App.Lang.useThemeDefaultBodyImg +'</label></li>');
		//arr.push('<li><input name="ct_body_bgi_type" id="ipt_hideBodyImg" type="radio" value="2" /><label for="ipt_hideBodyImg">'+ App.Lang.hideBodyImg +'</label></li>');
		//arr.push('<li><input name="ct_body_bgi_type" id="ipt_useMyBodyImg" type="radio" value="3" /><label for="ipt_useMyBodyImg">'+ App.Lang.useMyBodyImg +'</label> ');
		...
		_ipts = _ipts.concat($A(divCon.getElementsByTagName('select')));
		$A(_ipts).each(function(ipt) {
			/*if (ipt.id == 'ipt_useThemeDefaultBodyImg') {
				this.ipt_useThemeDefaultBodyImg = ipt;
			}
			if (ipt.id == 'ipt_hideBodyImg') {
				this.ipt_hideBodyImg = ipt;
			}
			if (ipt.id == 'ipt_useMyBodyImg') {
				this.ipt_useMyBodyImg = ipt;
			}*/
	...
	updateCusBodyData: function() {
		if (this.getStyle('body_bgi') && this.getStyle('body_bgi') == 'transparent') {
			//this.ipt_hideBodyImg.checked = true;
			this.activeOneBodyBgi('none');
		}
		else if (this.getStyle('body_bgi') != '') {
			//this.ipt_useMyBodyImg.checked = true;
			...
			//this.ipt_useThemeDefaultBodyImg.checked = true;
	...
	observeCusBody: function() {
		...
		/*Event.observe(this.ipt_useThemeDefaultBodyImg, 'click', function() {
			this.setCss('body', 'background-image', '');
		}.bind(this));
		Event.observe(this.ipt_hideBodyImg, 'click', function() {
			this.setCss('body', 'background-image', 'none');
		}.bind(this));
		Event.observe(this.ipt_useMyBodyImg, 'click', function() {
			if (this.getCss('body-bak', 'background-image')) {
				this.setCss('body', 'background-image', this.getCss('body-bak', 'background-image'));
			}
		}.bind(this));*/
	...
	analyesCss: function() {
		...
		if (_styleTag) {
			...
			$A(_rules).each(function(r) {
				...
				$A(_selectors).each(function(slt) {
					var _selector = slt.replace(/(^\s*)|(\s*$)/g, '');
					if (!this.styles[_selector]) {
						this.styles[_selector] = {};
					}
					var _styleSets = r.style;*/
					/*for (p in _styleSets) {
						if (_styleSets[p] && p != 'cssText') {
					...
					$H(_styleSets).each(function(s) {
						if (s.value && s.key != 'cssText' && (typeof s.value == 'string') && (s.key.indexOf('Moz')!=0) && isNaN(s.key-0)) {
							this.styles[_selector][s.key] = s.value;
							//$LR(_selector +'{'+ s.key +':'+ s.value +'}');
						}
					}.bind(this));
				}.bind(this));
	...
	*/
	...
	//	判断是否可以分享该主题
	/*
	canShareTheme: function() {
		var themeCss = $("themeCss");
		return !!$('customTheme') && themeCss && (typeof(themeCss.getAttribute("themeid")) == "string") && (themeCss.getAttribute("themeid").indexOf("g") != 0);
	},
	*/
	clean: function() {
	...
	//	点取消按钮时调用的方法
	cancel: function() {
		//$LR(JSON.stringify(this.stylesTempl));
		$H(this.stylesTempl).each(function(r) {
			$H(r.value).each(function(p) {
				//$LR('this.setCss('+ r.key +', '+ p.key.underscore().dasherize() +', '+ (this.styles[r.key]? (this.styles[r.key][p.key] || '') : '') +')')
...
PopWin.prototype = {
	...
	build: function() {
		...
		//Element.addClassName(divPopWin, 'popWin');
		
		var divInnerPopWin = document.createElement('div');
		divInnerPopWin.className = 'mod';
		//Element.addClassName(divInnerPopWin, 'mod');
		...
		//Element.addClassName(divPopWinFrame, 'modFrame');
		...
		//divClose.innerHTML = '<img src="'+App.Actions.imgPath+'spacer.gif" alt="'+ 
			//App.Lang.close +'" title="'+ App.Lang.delModuleBtnTitle +'" />';
		Event.observe(divClose, 'mousedown', this.eventCancelBubble);
	...
	buildBtn: function() {
		...
		//Element.addClassName(divBtn, 'divBtn');
		
		var btnOk = document.createElement('input');
		this.okBtn = btnOk;
		btnOk.className = 'button-submit';
		//Element.addClassName(btnOk, 'button-submit');
		btnOk.type = 'button';
		//btnOk.setAttribute('type', 'button');
		btnOk.value = this.options.okText;
		//btnOk.setAttribute('value', this.options.okText);
		...
		if (this.options.type == 'confirm') {
			...
			//Element.addClassName(btnCancel, 'button');
			btnCancel.type = 'button';
			//btnCancel.setAttribute('type', 'button');
			btnCancel.value = this.options.cancelText;
			//btnCancel.setAttribute('value', this.options.cancelText);
...
App.ToolTip = function(node, txt, w, align, ev) {
	...
	this.display = function(node, txt, w, align){
		...
		//(Browser.ua.indexOf('ie')>=0)? 
			//Element.show(tipobj) : 
			//Effect.Appear(tipobj, { duration: 0.3, queue: {scope: tipobj.id, position: 'end'} });
	...
	this.hide = function(){
		...
		//(Browser.ua.indexOf('ie')>=0)? 
			//Element.hide(tipobj) : 
			//Effect.Fade(tipobj, { duration: 0.0, queue:{scope: tipobj.id, position: 'end'} });
	};
